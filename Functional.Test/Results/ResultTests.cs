...
public class ResultTests
{
	...
	public void ItShouldBindSuccessesToSuccesses() =>
		1.Success()
			...
			.ShouldBeOfType(typeof(SuccessResult<string>));

	[TestMethod]
	public void ItShouldBindSuccessesToFailures() =>
		1.Success()
			.Bind(_ => Result.Failure<string>("oh no"))
			.ShouldBeOfType(typeof(FailureResult<string>));
	...
	[TestMethod]
	public void ItShouldThrowExceptionsForNonResultFakers() =>
		Assert.ThrowsException<InvalidOperationException>(
			() =>
				new FakeResult<int>(1)
					.Match(
						success => success.ToString(),
						failure => failure.FailureMessages.First())
					.Ignore());
...
internal record FakeResult<T>(T Contents) : Result<T>;