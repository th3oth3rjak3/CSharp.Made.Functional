...
public static class ExceptionExtensions
{
	...
	public static TResult Match<TInput, TResult>(
		this TryResult<TInput> tryResult,
		Func<TInput, TResult> onSuccess,
		Func<Exception, TResult> onFailure) =>
			tryResult switch
			{
				TrySuccess<TInput> success => onSuccess(success.Contents),
				TryFailure<TInput> failure => onFailure(failure.Exception),
...
public abstract record TryResult<T>();
...
public static class TryResult
{
	...
	public static TryResult<T> Failure<T>(Exception exception) =>
		new TryFailure<T>(exception);
}

[ExcludeFromCodeCoverage]
public record TrySuccess<T>(T Contents) : TryResult<T>;

[ExcludeFromCodeCoverage]
public record TryFailure<T>(Exception Exception) : TryResult<T>;