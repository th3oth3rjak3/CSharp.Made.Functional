...
public sealed record Union<A, B, C, D, E, F, G>
{
    /// <summary>
    ...
    public T Match<T>(Func<A, T> whenA, Func<B, T> whenB, Func<C, T> whenC, Func<D, T> whenD, Func<E, T> whenE, Func<F, T> whenF, Func<G, T> whenG)
    {
    ...
    public Unit Effect(Action<A> whenA, Action<B> whenB, Action<C> whenC, Action<D> whenD, Action<E> whenE, Action<F> whenF, Action<G> whenG)
    {
