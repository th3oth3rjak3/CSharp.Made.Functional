...
/// <summary>
/// Represents a Discriminated Union which contains five Variants.
/// </summary>
...
public sealed record Union<A, B, C, D, E>
{
    /// <summary>
    ...
    public T Match<T>(Func<A, T> whenA, Func<B, T> whenB, Func<C, T> whenC, Func<D, T> whenD, Func<E, T> whenE)
    {
    ...
    public Unit Effect(Action<A> whenA, Action<B> whenB, Action<C> whenC, Action<D> whenD, Action<E> whenE)
    {
